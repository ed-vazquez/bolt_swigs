defmodule Bolt.Swigs.Routing.TransactionTest do
  use Bolt.Swigs.RoutingConnCase
  @moduletag :routing

  setup do
    {:ok, [write_conn: Bolt.Swigs.conn(:write)]}
  end

  test "execute statements in transaction", %{write_conn: write_conn} do
    Bolt.Swigs.transaction(write_conn, fn conn ->
      book =
    ...
    books =
      Bolt.Swigs.query!(write_conn, "MATCH (b:Book {title: \"The Game Of Trolls\"}) return b")
  ...
  test "rollback statements in transaction", %{write_conn: write_conn} do
    try do
      ...
      {:ok, [result]} = Bolt.Swigs.query(write_conn, "MATCH (x:XactRollback) RETURN count(x)")
    ...
    after
      ...
      Bolt.Swigs.query(write_conn, "MATCH (x:XactRollback) DETACH DELETE x")
    end
  end
  ...
  test "commit statements in transaction", %{write_conn: write_conn} do
    try do
      Bolt.Swigs.transaction(write_conn, fn conn ->
        book = Bolt.Swigs.query(conn, "CREATE (x:XactCommit {foo: 'bar'}) return x")
        assert {:ok, [row]} = book
        assert row["x"].properties["foo"] == "bar"

        # Main connection should not see this new node.
        {:ok, results} = Bolt.Swigs.query(write_conn, "MATCH (x:XactCommit) RETURN x")
      ...
      {:ok, [%{"x" => node}]} = Bolt.Swigs.query(write_conn, "MATCH (x:XactCommit) RETURN x")
      assert node.labels == ["XactCommit"]
      assert node.properties["foo"] == "bar"
    after
      # Delete any XactCommit nodes that were succesfully committed!
      Bolt.Swigs.query(write_conn, "MATCH (x:XactCommit) DETACH DELETE x")
    end
  end
end
