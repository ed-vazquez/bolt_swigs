defmodule Bolt.Swigs.PerformanceTest do
  use Bolt.Swigs.ConnCase, async: false
  ...
  test "Querying 500 nodes should under 100ms", context do
    ...
    assert %Bolt.Swigs.Response{stats: %{"nodes-created" => 500}} =
             Bolt.Swigs.query!(Bolt.Swigs.conn(), cql_create)
    ...
    output =
      Benchee.run(
        %{
          ...
          "run" => fn -> Bolt.Swigs.Query.query(conn, simple_cypher) end
          # " new conn" => fn -> query.(Bolt.Swigs.conn(), simple_cypher) end
        },
        time: 5
      )
      ...
      Benchee.run(
        %{
          ...
          "run with properties" => fn ->
            Bolt.Swigs.Query.query(conn, simple_cypher, %{
