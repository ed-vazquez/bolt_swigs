defmodule Bolt.Swigs.Internals.BoltProtocolV3Test do
  ...
  describe "Transaction management" do
    ...
    test "With socket instead of gent_tcp", %{config: config, port: port} do
      assert {:ok, _} = BoltProtocolV3.hello(Bolt.Swigs.Socket, port, 3, config[:auth], [])
      {:ok, metadata} = Metadata.new(%{bookmarks: ["neo4j:bookmark:v1:tx234"], tx_timeout: 1_000})

      {:ok, _} = BoltProtocolV3.begin(Bolt.Swigs.Socket, port, 3, metadata, [])
    end
  end
end
