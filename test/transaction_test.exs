defmodule Transaction.Test do
  ...
  test "execute statements in transaction", %{main_conn: main_conn} do
    ...
    books =
      Bolt.Swigs.query!(main_conn, "MATCH (b:Book {title: \"The Game Of Trolls\"}) return b")
  ...
  test "rollback statements in transaction", %{main_conn: main_conn} do
    try do
      ...
      {:ok, %Response{results: [result]}} =
        Bolt.Swigs.query(main_conn, "MATCH (x:XactRollback) RETURN count(x)")
    ...
    after
      ...
      Bolt.Swigs.query(main_conn, "MATCH (x:XactRollback) DETACH DELETE x")
    end
  end
  ...
  test "commit statements in transaction", %{main_conn: main_conn} do
    try do
      Bolt.Swigs.transaction(main_conn, fn conn ->
        assert {:ok, %Response{results: books}} =
                 Bolt.Swigs.query(conn, "CREATE (x:XactCommit {foo: 'bar'}) return x")
        ...
        {:ok, %Response{results: results}} =
          Bolt.Swigs.query(main_conn, "MATCH (x:XactCommit) RETURN x")
    ...
    after
      ...
      Bolt.Swigs.query(main_conn, "MATCH (x:XactCommit) DETACH DELETE x")
    end
  end
end
