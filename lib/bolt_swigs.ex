defmodule Bolt.Swigs do
  ...
  To start, add the `:bolt_swigs` dependency to you project, run `mix do deps.get, compile` on it and then you can quickly start experimenting with Neo4j from the convenience of your IEx shell. Example:
  ...
  @registry_name :bolt_swigs_registry

  # @timeout 15_000
  # @max_rows     500

  alias Bolt.Swigs.{Query, ConnectionSupervisor, Router, Error, Response, Exception}
  ...
  Example with a configuration defined in the `config/dev.exs`:
      ...
      opts = Application.get_env(:bolt_swigs, Bolt)
      {:ok, pid} = Bolt.Swigs.start_link(opts)

      Bolt.Swigs.query!(pid, "CREATE (a:Person {name:'Bob'})")
      Bolt.Swigs.query!(pid, "MATCH (a:Person) RETURN a.name AS name")
      |> Enum.map(&(&1["name"]))
  ...
  Example with a configuration defined in the `config/dev.exs`:
      ...
      {:ok, _neo} = Bolt.Swigs.start_link(url: "bolt://neo4j:test@localhost")

      conn = Bolt.Swigs.conn()
      Bolt.Swigs.query!(conn, "return 1 as n")
  ...
  @doc """
    ...
    ```elixir
    setup do
      {:ok, [main_conn: Bolt.Swigs.conn()]}
    end

    test "execute statements in transaction", %{main_conn: main_conn} do
      Bolt.Swigs.transaction(main_conn, fn conn ->
        book =
      ...
      books = Bolt.Swigs.query!(main_conn, "MATCH (b:Book {title: \"The Game Of Trolls\"}) return b")
  ...
  ### Example
      ...
      {:error, :oops} = Bolt.Swigs.transaction(pool, fn(conn) ->
        Bolt.Swigs.rollback(conn, :oops)
      end)
  """
  ...
  iex> Bolt.Swigs.info()
  %{default: %{connections: %{direct: %{"localhost:7687" => 0}, routing_query: nil, zorba: %{"localhost:7687" => 0}}, user_options: [basic_auth: [username: "******", password: "******"], socket: Bolt.Swigs.Socket, port: 7687, routing_context: %{}, schema: "bolt", hostname: "localhost", timeout: 15000, ssl: false, with_etls: false, prefix: :default, url: "bolt://localhost", pool_size: 10, max_overflow: 2, role: :zorba]}}
  """
  @spec info() :: map
  def info(), do: sanitized_info(Bolt.Swigs.Router.info())
  ...
  def routing_table(prefix) do
    Bolt.Swigs.Router.routing_table(prefix)
  end

  @doc """
  the registry name used across the various driver components
  """
  @spec registry_name() :: :bolt_swigs_registry
