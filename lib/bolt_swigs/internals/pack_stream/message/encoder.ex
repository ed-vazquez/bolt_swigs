defmodule Bolt.Swigs.Internals.PackStream.Message.Encoder do
  @moduledoc false
  _module_doc = """
  ...
  All messages are serialized structures. See `Bolt.Swigs.Internals.PackStream.EncoderV1` for
  ...
  @spec signature(Bolt.Swigs.Internals.PackStream.Message.out_signature()) :: integer()
  ...
  """
  def client_name() do
    "BoltSwigs/" <> to_string(Application.spec(:bolt_swigs, :vsn))
  end
  ...
  # Encode HELLO with auth for Bolt V4 (same as V3)
  def encode({:hello, []}, 4) do
    encode({:hello, [{}]}, 4)
  end

  def encode({:hello, [auth]}, 4) do
    do_encode(:hello, [auth_params(auth)], 4)
  end

  # Encode BEGIN for Bolt V4 (same as V3)
  def encode({:begin, []}, 4) do
    encode({:begin, [%{}]}, 4)
  end

  def encode({:begin, [%Metadata{} = metadata]}, 4) do
    do_encode(:begin, [Metadata.to_map(metadata)], 4)
  end

  def encode({:begin, [%{} = map]}, 4) when map_size(map) == 0 do
    {:ok, metadata} = Metadata.new(%{})
    encode({:begin, [metadata]}, 4)
  end

  # Encode RUN for Bolt V4 (same as V3, supports metadata)
  def encode({:run, [statement]}, 4) do
    do_encode(:run, [statement, %{}, %{}], 4)
  end

  def encode({:run, [statement, params]}, 4) do
    do_encode(:run, [statement, params, %{}], 4)
  end

  def encode({:run, [statement, params, %Metadata{} = metadata]}, 4) do
    do_encode(:run, [statement, params, Metadata.to_map(metadata)], 4)
  end

  # Encode PULL with n parameter for Bolt V4
  def encode({:pull, [%{} = extra]}, 4) do
    do_encode(:pull, [extra], 4)
  end

  # Encode DISCARD with n parameter for Bolt V4
  def encode({:discard, [%{} = extra]}, 4) do
    do_encode(:discard, [extra], 4)
  end

  # INIT is not valid in Bolt V4
  def encode({:init, _}, 4) do
    {:error, :invalid_message}
  end

  # Encode messages that don't need any data formating for Bolt V4
  def encode({message_type, data}, 4) when message_type in @valid_message_types do
    do_encode(message_type, data, 4)
  end

  # Encode messages for Bolt V5

  # INIT and ACK_FAILURE are no longer valid in Bolt V5
  def encode({:init, _}, bolt_version) when bolt_version >= 5 do
    {:error, :invalid_message}
  end

  def encode({:ack_failure, _}, bolt_version) when bolt_version >= 5 do
    {:error, :invalid_message}
  end

  # Encode HELLO without auth for Bolt V5 (same as V3)
  def encode({:hello, []}, bolt_version) when bolt_version >= 5 do
    encode({:hello, [{}]}, bolt_version)
  end

  # Encode HELLO with auth for Bolt V5
  def encode({:hello, [auth]}, bolt_version) when bolt_version >= 5 do
    do_encode(:hello, [auth_params(auth)], bolt_version)
  end

  # Encode LOGON with auth token for Bolt V5
  def encode({:logon, [auth]}, bolt_version) when bolt_version >= 5 do
    do_encode(:logon, [auth_params_v5(auth)], bolt_version)
  end

  # Encode LOGOFF for Bolt V5
  def encode({:logoff, []}, bolt_version) when bolt_version >= 5 do
    do_encode(:logoff, [], bolt_version)
  end

  # Encode RUN for Bolt V5 (same as V3)
  def encode({:run, [statement]}, bolt_version) when bolt_version >= 5 do
    do_encode(:run, [statement, %{}, %{}], bolt_version)
  end

  def encode({:run, [statement, params]}, bolt_version) when bolt_version >= 5 do
  ...
  # Encode BEGIN for Bolt V5
  def encode({:begin, []}, bolt_version) when bolt_version >= 5 do
    encode({:begin, [%{}]}, bolt_version)
  end

  def encode({:begin, [%Metadata{} = metadata]}, bolt_version) when bolt_version >= 5 do
    do_encode(:begin, [Metadata.to_map(metadata)], bolt_version)
  end

  def encode({:begin, [%{} = map]}, bolt_version) when bolt_version >= 5 and map_size(map) == 0 do
    {:ok, metadata} = Metadata.new(%{})
    encode({:begin, [metadata]}, bolt_version)
  end

  # Encode PULL with n parameter for Bolt V5
  def encode({:pull, [%{} = extra]}, bolt_version) when bolt_version >= 5 do
    do_encode(:pull, [extra], bolt_version)
  end

  # Encode DISCARD with n parameter for Bolt V5
  def encode({:discard, [%{} = extra]}, bolt_version) when bolt_version >= 5 do
    do_encode(:discard, [extra], bolt_version)
  end

  # Encode ROUTE for Bolt V5
  def encode({:route, [routing_context, bookmarks, db]}, bolt_version) when bolt_version >= 5 do
    do_encode(:route, [routing_context, bookmarks, db], bolt_version)
  end

  # Encode TELEMETRY for Bolt V5
  def encode({:telemetry, [api]}, bolt_version) when bolt_version >= 5 do
    do_encode(:telemetry, [api], bolt_version)
  end

  # Encode messages that don't need any data formating for Bolt V5
  def encode({message_type, data}, bolt_version)
      when bolt_version >= 5 and message_type in @valid_message_types do
    do_encode(message_type, data, bolt_version)
  end
  ...
  Example
      ...
      iex> alias Bolt.Swigs.Internals.PackStream.Message.Encoder
  ...
  | data | type |
  |------|------|
  | metadata | See Bolt.Swigs.Metadata
  ...
  Examples (excluded from doctest because client_name changes at each bolt_sips version)
      ...
      # without auth token
      diex> alias Bolt.Swigs.Internals.PackStream.Message.Encoder
  ...
  # Format the auth params for Bolt V5 (LOGON message)
  @spec auth_params_v5({} | {String.t(), String.t()} | map()) :: map()
  defp auth_params_v5(%{} = auth_map), do: auth_map
  defp auth_params_v5({}), do: %{}

  defp auth_params_v5({username, password}) do
    %{
      scheme: "basic",
      principal: username,
      credentials: password
  ...
  """
  @spec encode_message(
          Bolt.Swigs.Internals.PackStream.Message.out_signature(),
          integer(),
          list(),
          integer()
        ) ::
  ...
  def encode_message(message_type, signature, data, bolt_version) do
    Bolt.Swigs.Internals.Logger.log_message(:client, message_type, data)

    encoded =
      {signature, data}
      |> Bolt.Swigs.Internals.PackStream.encode(bolt_version)
      |> generate_chunks([])

    Bolt.Swigs.Internals.Logger.log_message(:client, message_type, encoded, :hex)
    encoded
  end

  @spec generate_chunks(Bolt.Swigs.Internals.PackStream.value() | <<>>, list()) ::
          [[Bolt.Swigs.Internals.PackStream.Message.encoded()]]
  ...
  @spec format_chunk(Bolt.Swigs.Internals.PackStream.value()) ::
          [Bolt.Swigs.Internals.PackStream.Message.encoded()]
