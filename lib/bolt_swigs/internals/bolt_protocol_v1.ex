defmodule Bolt.Swigs.Internals.BoltProtocolV1 do
  @moduledoc false
  alias Bolt.Swigs.Internals.BoltProtocolHelper
  alias Bolt.Swigs.Internals.BoltVersionHelper
  ...
  @spec handshake(atom(), port(), Keyword.t()) ::
          {:ok, integer()} | {:error, Bolt.Swigs.Internals.Error.t()}
  def handshake(transport, port, options \\ [recv_timeout: 15_000]) do
    ...
    Bolt.Swigs.Internals.Logger.log_message(
      :client,
      :handshake,
    ...
    case transport.recv(port, 4, recv_timeout) do
      {:ok, <<version::32>> = packet} when version <= max_version ->
        Bolt.Swigs.Internals.Logger.log_message(:server, :handshake, packet, :hex)
        Bolt.Swigs.Internals.Logger.log_message(:server, :handshake, version)
        {:ok, version}
  ...
  ## Examples
      ...
      iex> Bolt.Swigs.Internals.BoltProtocol.init(:gen_tcp, port, 1, {}, [])
      {:ok, info}

      iex> Bolt.Swigs.Internals.BoltProtocol.init(:gen_tcp, port, 1, {"username", "password"}, [])
      {:ok, info}
  """
  ...
  @spec pull_all(atom(), port(), integer(), Keyword.t()) ::
          {:ok, list()}
          | {:failure, Bolt.Swigs.Internals.Error.t()}
          | {:failure, Bolt.Swigs.Internals.Error.t()}
  ...
  ## Examples
      ...
      iex> Bolt.Swigs.Internals.BoltProtocol.run_statement(:gen_tcp, port, 1, "MATCH (n) RETURN n")
      [
        {:success, %{"fields" => ["n"]}},
  ...
  ## Example
      ...
      :ok
  """
  @spec discard_all(atom(), port(), integer(), Keyword.t()) ::
          :ok | Bolt.Swigs.Internals.Error.t()
  ...
  ## Example
      ...
      :ok
  """
  @spec ack_failure(atom(), port(), integer(), Keyword.t()) ::
          :ok | Bolt.Swigs.Internals.Error.t()
  ...
  ## Example
      ...
      :ok
  """
  @spec reset(atom(), port(), integer(), Keyword.t()) :: :ok | Bolt.Swigs.Internals.Error.t()
