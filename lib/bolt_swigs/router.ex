defmodule Bolt.Swigs.Router do
  @moduledoc """
  ...
  alias Bolt.Swigs.Routing.RoutingTable
  alias Bolt.Swigs.{Protocol, ConnectionSupervisor, LoadBalancer, Response, Error}
  ...
  defp _configure(opts) do
    options = Bolt.Swigs.Utils.default_config(opts)
  ...
  defp get_routing_table(
    ...
    with {:ok, conn, updated_connections} <- _get_connection(:route, connections, prefix),
         {:ok, %Response{} = results} <- Bolt.Swigs.query(conn, query, props) do
  ...
  defp get_routing_table(opts, _) do
    ...
    with {:ok, %Protocol.ConnData{configuration: configuration}} <- Protocol.connect(opts),
      ...
      with {:ok, pid} <- DBConnection.start_link(Protocol, Keyword.delete(opts, :name)),
           {:ok, %Response{} = results} <- Bolt.Swigs.query(pid, query, params),
  ...
  def start_connections(opts, routing_table) do
    connections =
      with %Bolt.Swigs.Routing.RoutingTable{roles: roles} = rt <-
             RoutingTable.parse(routing_table) do
        roles
  ...
  def handle_info({:refresh, prefix}, state) do
    ...
    state =
      with {:ok, routing_table, _updated_connections} <-
        ...
        connections =
          connections
          |> Map.put(:updated_at, Bolt.Swigs.Utils.now())
  ...
  iex> Bolt.Swigs.Router.parse_server_version(%{"server" => "Neo4j/3.5.0"})
  {"Neo4j/3.5.0", "3.5.0"}

  iex> Bolt.Swigs.Router.parse_server_version(%{"server" => "Neo4j/3.5"})
  {"Neo4j/3.5", "3.5.0"}

  iex> Bolt.Swigs.Router.parse_server_version(%{"server" => "Neo4j/3.5.10"})
  {"Neo4j/3.5.10", "3.5.10"}

  iex> Bolt.Swigs.Router.parse_server_version(%{"server" => "Neo4j/3.5.11.1"})
