defmodule Bolt.Swigs.Protocol do
  @moduledoc false
  ...
  alias Bolt.Swigs.QueryStatement
  alias Bolt.Swigs.Internals.Error, as: BoltError
  alias Bolt.Swigs.Internals.BoltProtocol

  @doc "Callback for DBConnection.connect/1"

  def connect(opts \\ [])
  def connect([]), do: connect(Bolt.Swigs.Utils.default_config())

  def connect(opts) do
    conf = opts |> Bolt.Swigs.Utils.default_config()
  ...
  defp execute(q, params, _, conn_data) do
    ...
    e ->
      msg =
        case e do
          %Bolt.Swigs.Internals.PackStreamError{} = e ->
