defmodule Bolt.Swigs.Query do
  @moduledoc """
  Provides a simple Query DSL.

  You can run simple Cypher queries with or w/o parameters, for example:

      {:ok, row} = Bolt.Swigs.query(conn, "match (n:Person {bolt_sips: true}) return n.name as Name limit 5")
  ...
  Or more complex ones:
      ...
      \"""
      {:ok, r} = Bolt.Swigs.query(conn, cypher)
  ...
  For example, say you want to clean up the test database **before** creating some tests entities. You can do that like this:
      ...
      CREATE
        ...
        (Patrick)-[:WROTE]->(TNOTW)
      \"""
      assert {:ok, _r} = Bolt.Swigs.query(conn, cypher)
  ...
  """
  alias Bolt.Swigs.{QueryStatement, Response, Types, Error, Exception}

  @cypher_seps ~r/;(.){0,1}\n/

  @spec query!(Bolt.Swigs.conn(), String.t()) :: Response.t() | Exception.t()
  def query!(conn, statement), do: query!(conn, statement, %{})

  @spec query!(Bolt.Swigs.conn(), String.t(), map, Keyword.t()) :: Response.t() | Exception.t()
  ...
  @spec query(Bolt.Swigs.conn(), String.t()) :: {:error, Error.t()} | {:ok, Response.t()}
  def query(conn, statement), do: query(conn, statement, %{})

  @spec query(Bolt.Swigs.conn(), String.t(), map, Keyword.t()) ::
  ...
  def query(conn, statement, params, opts \\ []) when is_map(params) do
    ...
    e in Bolt.Swigs.Exception ->
      {:error, %Bolt.Swigs.Error{code: e.code, message: e.message}}
  end

  ###########
  # Private #
  ###########
  @spec query_commit(Bolt.Swigs.conn(), String.t(), map, Keyword.t()) ::
          {:error, Error.t()} | {:ok, Response.t()}
  defp query_commit(conn, statement, params, opts) do
    ...
    e in [RuntimeError, DBConnection.ConnectionError] ->
      {:error, Bolt.Swigs.Error.new(e.message)}
  ...
  defp send!(conn, statement, params, opts, acc) do
    ...
    case DBConnection.execute(conn, %QueryStatement{statement: statement}, params, opts) do
      ...
      {:error, %Bolt.Swigs.Internals.Error{code: code, message: msg}} ->
