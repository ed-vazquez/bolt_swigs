defmodule Bolt.Swigs.Response do
  @moduledoc """
  Support for transforming a Bolt response to a list of Bolt.Swigs.Types or arbitrary values.
  A Bolt.Swigs.Response is used for mapping any response received from a Neo4j server into a an Elixir struct.
  ...
  For example, a simple Cypher query like this:
      ...
      Bolt.Swigs.query(Bolt.Swigs.conn(), "RETURN [10,11,21] AS arr")
  ...
  and while you have now access to the full data returned by Neo4j, most of the times you'll just want the results:
      ...
      iex> %Bolt.Swigs.Response{results: results} = Bolt.Swigs.query!(Bolt.Swigs.conn(), "RETURN [10,11,21] AS arr")
      iex> results
      [%{"arr" => [10, 11, 21]}]
  ...
  will return `results` like this:
      ...
      [%{"r" => %Bolt.Swigs.Types.Relationship{end: 647, id: 495,
  ...
  Our Bolt.Swigs.Response is implementing Elixir's [Enumerable Protocol](https://hexdocs.pm/elixir/Enumerable.html), to help you accessing the results. Hence something like this, is possible:

      iex> Bolt.Swigs.query!(Bolt.Swigs.conn(), "RETURN [10,11,21] AS arr") |>
  ...
  You can also quickly get the `first` of the results, returned by Neo4j. Example:
      ...
      iex> Bolt.Swigs.query!(Bolt.Swigs.conn(), "UNWIND range(1, 10) AS n RETURN n") |>
  ...
  def transform(records, _stats \\ :no) do
    ...
    e in Bolt.Swigs.Exception -> {:error, e.message}
    e -> {:error, e}
  end

  def fetch(%Bolt.Swigs.Response{fields: fields, results: results}, key) do
  ...
  def fetch!(%Bolt.Swigs.Response{} = r, key) do
  ...
  defp parse_record(:success, record, _response) do
    ...
    {:error, Bolt.Swigs.Error.new(err_msg)}
  end
