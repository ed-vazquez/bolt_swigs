defmodule Bolt.Swigs.ConnectionSupervisor do
  @moduledoc false

  use DynamicSupervisor

  alias Bolt.Swigs.Protocol
  alias Bolt.Swigs
  ...
  def find_connection(name) do
    case Registry.lookup(Swigs.registry_name(), name) do
      [{pid, _}] -> {:ok, pid}
      _ -> {:error, :not_found}
    end
  end
  ...
  def via_tuple(name) do
    {:via, Registry, {Bolt.Swigs.registry_name(), name}}
  end
end
